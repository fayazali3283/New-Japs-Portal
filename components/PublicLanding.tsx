...
const PublicLanding: React.FC<PublicLandingProps> = ({ onEnterPortal }) => {
  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-slate-300 font-sans selection:bg-blue-500/30 overflow-x-hidden">
      {/* Dynamic Background Glows */}
      <div className="fixed top-[-10%] left-[-10%] w-[50%] h-[50%] bg-blue-600/10 rounded-full blur-[120px] pointer-events-none opacity-50"></div>
      <div className="fixed bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-purple-600/10 rounded-full blur-[120px] pointer-events-none opacity-50"></div>

      {/* Navigation */}
      <nav className="relative z-20 flex justify-between items-center px-6 md:px-12 py-6 md:py-8 max-w-7xl mx-auto backdrop-blur-xl bg-white/[0.02] border-b border-white/5 sticky top-0">
        <button 
          onClick={() => window.location.reload()}
          className="flex items-center space-x-3 group"
        >
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white w-10 h-10 md:w-12 md:h-12 rounded-2xl flex items-center justify-center font-black text-xl md:text-2xl shadow-[0_0_30px_rgba(59,130,246,0.4)] group-hover:scale-110 transition-transform">J</div>
        ...
        <div className="flex items-center space-x-4 md:space-x-10">
          ...
          <button 
            ...
            className="bg-white text-[#0a0c10] px-5 md:px-8 py-2.5 md:py-3.5 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-[0_0_40px_rgba(255,255,255,0.1)]"
          >
            Access Portal
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-center">
          <div className="text-center lg:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-6">
              <button 
                onClick={onEnterPortal}
                className="w-full sm:w-auto bg-gradient-to-br from-blue-500 to-indigo-600 text-white px-10 md:px-14 py-5 md:py-6 rounded-[2rem] font-black text-sm md:text-base uppercase tracking-widest shadow-[0_20px_60px_rgba(59,130,246,0.3)] hover:-translate-y-1 active:translate-y-0 transition-all"
              >
          ...
          <div className="relative group perspective-1000">
            ...
            <div className="relative bg-white/5 backdrop-blur-3xl border border-white/10 p-8 md:p-14 rounded-[3.5rem] md:rounded-[4.5rem] shadow-2xl transition-transform duration-700 group-hover:rotate-y-12">
              ...
              <div className="space-y-8 mb-12">
                {[
                  ...
                  <div key={idx} className="flex items-center space-x-5 text-blue-400 font-bold">
                    <span className="flex-shrink-0 w-6 h-6 rounded-lg bg-blue-500/10 flex items-center justify-center text-[10px]">✓</span>
                    <span className="text-base md:text-lg tracking-tight">{text}</span>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-3 gap-4">
                {['Registry', 'Faculty', 'Management'].map(role => (
                  <div key={role} className="bg-white/5 border border-white/10 p-4 rounded-3xl text-center hover:bg-white/10 transition-colors">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">{role}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mt-40">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {schoolFeatures.map((feature, i) => (
              <div key={i} className="bg-white/[0.02] border border-white/5 rounded-[3rem] p-10 hover:border-blue-500/40 transition-all hover:bg-white/[0.05] group">
                <div className="text-5xl mb-8 group-hover:scale-110 group-hover:-rotate-6 transition-transform inline-block">{feature.icon}</div>
                <h4 className="text-blue-400 font-black text-xl mb-6 tracking-widest uppercase">{feature.title}</h4>
                <ul className="space-y-4">
                  {feature.items.map((item, j) => (
                    <li key={j} className="text-slate-500 font-bold flex items-center text-sm md:text-base group-hover:text-slate-300 transition-colors">
                      <span className="w-1.5 h-1.5 bg-blue-600/60 rounded-full mr-4 group-hover:scale-150 transition-transform"></span> {item}
                    </li>
                  ))}
                </ul>
              </div>

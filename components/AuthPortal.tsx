...
const AuthPortal: React.FC<AuthPortalProps> = ({ onLogin, onBack }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#07090e] p-2 md:p-6 font-sans">
      <div className="bg-[#11141b] rounded-[2.5rem] md:rounded-[4rem] shadow-2xl w-full max-w-6xl overflow-hidden flex flex-col lg:flex-row border border-white/10 relative">
        {/* Glow effect */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-600/10 rounded-full blur-[120px] pointer-events-none"></div>

        {/* Branding Side */}
        <div className="hidden lg:flex lg:w-[40%] bg-[#1E3A8A] p-12 text-white flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            <button onClick={onBack} className="mb-12 flex items-center text-blue-300 font-black text-[10px] uppercase tracking-[0.3em] hover:text-white transition-colors group">
        ...
        {/* Content Side */}
        <div className="w-full lg:w-[60%] p-8 md:p-16 lg:p-24 bg-[#11141b] flex flex-col justify-center text-slate-300 h-full overflow-y-auto">
          <button onClick={onBack} className="lg:hidden mb-10 flex items-center text-blue-500 font-black text-[10px] uppercase tracking-[0.3em]">
            ← Return
          </button>
          ...
          <div className="max-w-md mx-auto w-full">
            <div className="flex bg-black/40 p-1.5 rounded-2xl md:rounded-[2rem] mb-12 border border-white/5 shadow-inner">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 py-3.5 md:py-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest rounded-xl md:rounded-[1.5rem] transition-all ${
                    mode === tab.id ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'
                  }`}
                >
                  {tab.label}
                </button>
            ...
            <div className="mb-10 text-center lg:text-left">
               <h3 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-3 leading-none uppercase">
                {mode === 'STAFF_LOGIN' ? 'Executive' : mode === 'STUDENT_REG' ? 'Enroll' : 'Access'}
               </h3>
               <p className="text-slate-500 font-medium tracking-tight">Institutional identity required for portal entry.</p>
            </div>

            <form onSubmit={handleAuth} className="space-y-6">
              {mode === 'STAFF_LOGIN' && (
                <div className="grid grid-cols-2 gap-3 mb-8 p-4 bg-black/20 rounded-3xl border border-white/5 animate-in slide-in-from-top-4 duration-500">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`py-3.5 px-3 text-[9px] md:text-[10px] font-black uppercase rounded-2xl border transition-all ${
                        selectedRole === role.role ? 'bg-blue-600/10 border-blue-500 text-blue-400' : 'bg-transparent border-white/5 text-slate-600 hover:border-white/10 hover:text-slate-400'
                      }`}
                    >
                      {role.label}
                    </button>
                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                  <div className="relative">
                    <input 
                      ...
                      className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl md:rounded-[1.75rem] text-sm text-white focus:border-blue-500 outline-none transition-all placeholder-slate-700"
                    />
                  </div>
                </div>
              )}
              ...
              <button 
                type="submit"
                className="w-full bg-blue-600 text-white py-6 rounded-2xl md:rounded-[2rem] font-black text-[11px] md:text-xs uppercase tracking-[0.4em] shadow-2xl shadow-blue-600/20 hover:scale-[1.02] active:scale-95 transition-all mt-8"
              >
